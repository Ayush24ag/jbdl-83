...
public class UserServiceTest {
    ...
    public void checkAvailabilityBookTest() {
     ...
     public void returnBookWithoutPenalty() {
    	 ...
    	 mockTransaction.setBorrowedDate(LocalDate.of(2025, 6, 10));
    	...
    	//verify ....
    	verify(penaltyRepo,times(0)).save(any(Penalty.class) );
     }
     @Test
     public void returnBookWithPenalty() {
    	 Transaction mockTransaction=new Transaction();
    	 mockTransaction.setTid(1);
    	 mockTransaction.setBorrowedDate(LocalDate.of(2025, 5, 14));
    	 mockTransaction.setStatus("BORROWED");
    	 Book mockBook=new Book();
     	mockBook.setBookId(101);
     	mockBook.setStock(1);
     	mockTransaction.setBook(mockBook);
     	//PRE-CONDITION ....
    	 when(transRepo.findById(1)).
    	 thenReturn(Optional.of(mockTransaction));
    	 //
    	Book b= userService.returnBook(1);
    	Penalty penalty=mockTransaction.getPenalty();
    	assertEquals(100, penalty.getAmount());
    	assertEquals(32, penalty.getNoOfDays());
    	assertEquals(2, b.getStock());
    	assertEquals("RETURNED", mockTransaction.getStatus());
